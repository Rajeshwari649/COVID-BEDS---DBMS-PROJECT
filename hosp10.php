...
<h1>BILLROTH HOSPITALS SHENOY NAGAR.</h1>

<h3><label class="add">ADDRESS- :</label>43, Lakshmi Talkies Rd, Shenoy Nagar, Chennai, Tamil Nadu 600030</h3>
<h2>BED AVAILABILITY</h2>

	
<table class="center">
                  <?php
				  	$result=mysqli_query($server,"SELECT total from beds where hosp_id=10 and  bed_type='covid_bed'");
					$row1=mysqli_fetch_row($result);
					
					$result1=mysqli_query($server,"SELECT occupied from beds where hosp_id=10 and  bed_type='covid_bed'");
					$row2=mysqli_fetch_row($result1);

					$result2=mysqli_query($server,"SELECT vacant from beds where hosp_id=10 and  bed_type='covid_bed'");
					$row3=mysqli_fetch_row($result2);

					$result3=mysqli_query($server,"SELECT total from beds where hosp_id=10 and  bed_type='oxy_bed'");
					$row4=mysqli_fetch_row($result3);
					
					$result4=mysqli_query($server,"SELECT occupied from beds where hosp_id=10 and  bed_type='oxy_bed'");
					$row5=mysqli_fetch_row($result4);

					$result5=mysqli_query($server,"SELECT vacant from beds where hosp_id=10 and  bed_type='oxy_bed'");
					$row6=mysqli_fetch_row($result5);
					
							$result6=mysqli_query($server,"SELECT total from beds where hosp_id=10 and  bed_type='non_oxy_bed'");
					$row7=mysqli_fetch_row($result6);
					
					$result7=mysqli_query($server,"SELECT occupied from beds where hosp_id=10 and  bed_type='non_oxy_bed'");
					$row8=mysqli_fetch_row($result7);

					$result8=mysqli_query($server,"SELECT vacant from beds where hosp_id=10 and  bed_type='non_oxy_bed'");
					$row9=mysqli_fetch_row($result8);

							$result9=mysqli_query($server,"SELECT total from beds where hosp_id=10 and  bed_type='icu_bed'");
					$row10=mysqli_fetch_row($result9);
					
					$result10=mysqli_query($server,"SELECT occupied from beds where hosp_id=10 and  bed_type='icu_bed'");
					$row11=mysqli_fetch_row($result10);

					$result11=mysqli_query($server,"SELECT vacant from beds where hosp_id=10 and  bed_type='icu_bed'");
					$row12=mysqli_fetch_row($result11);
	
							$result12=mysqli_query($server,"SELECT total from beds where hosp_id=10 and  bed_type='venti_bed'");
					$row13=mysqli_fetch_row($result12);
					
					$result13=mysqli_query($server,"SELECT occupied from beds where hosp_id=10 and  bed_type='venti_bed'");
					$row14=mysqli_fetch_row($result13);

					$result14=mysqli_query($server,"SELECT vacant from beds where hosp_id=10 and  bed_type='venti_bed'");
